...
export const SearchResults: React.FC<SearchResultsProps> = ({
  ...
  // Group results by distance if distance grouping is enabled
  const groupedResults = React.useMemo(() => {
    if (!showDistanceGrouping || results.every(r => r.distance === undefined)) {
      return { withinRadius: results, outsideRadius: [] }
    }

    const withinRadius = results.filter(r => r.withinRadius !== false)
    const outsideRadius = results.filter(r => r.withinRadius === false)

    return { withinRadius, outsideRadius }
  }, [results, showDistanceGrouping])

  const renderDistanceBadge = (distance?: number) => {
  ...
  if (view === 'list') {
    const renderListResults = (resultList: SearchResult[], isOutsideRadius = false) => (
      <div className={clsx('space-y-4', isOutsideRadius && 'opacity-75')}>
              ...
              <div className="flex gap-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    {result.distance !== undefined && renderDistanceBadge(result.distance)}
                    <div className="flex items-center gap-1">
    ...
    if (showDistanceGrouping && groupedResults.outsideRadius.length > 0) {
      return (
        <div className={className}>
          {groupedResults.withinRadius.length > 0 && (
            <>
              <h3 className="text-lg font-semibold mb-4">Im Umkreis von {searchRadius} km</h3>
              {renderListResults(groupedResults.withinRadius)}
            </>
          )}
          {groupedResults.outsideRadius.length > 0 && (
            <>
              <h3 className="text-lg font-semibold mt-8 mb-4 text-gray-600">
                Weitere Vorschläge außerhalb des Suchradius
              </h3>
              {renderListResults(groupedResults.outsideRadius, true)}
            </>
          )}
        </div>
      )
    ...
    return renderListResults(results)
  }

  const renderGridResults = (resultList: SearchResult[], isOutsideRadius = false) => (
    <div className={clsx(
      'grid gap-6',
      'grid-cols-1 md:grid-cols-2 lg:grid-cols-3',
      isOutsideRadius && 'opacity-75'
    )}>
      {resultList.map((result, index) => (
        <motion.div
          key={result.id}
          ...
          >
            <div className="relative aspect-[4/3] bg-gray-200">
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                <div className="bg-white/90 backdrop-blur text-eventhour-black px-3 py-1 rounded-full font-bold">
                  {result.price}€
                </div>
                {result.distance !== undefined && (
                  <div className="bg-white/90 backdrop-blur text-eventhour-black px-2 py-1 rounded-full text-xs">
    ...
    return (
      <div className={className}>
        {groupedResults.withinRadius.length > 0 && (
          <>
            <h3 className="text-lg font-semibold mb-4">Im Umkreis von {searchRadius} km</h3>
            {renderGridResults(groupedResults.withinRadius)}
          </>
        )}
        {groupedResults.outsideRadius.length > 0 && (
          <>
            ...
            {renderGridResults(groupedResults.outsideRadius, true)}
          </>
        )}
      </div>
    )
  ...
  return renderGridResults(results)
}